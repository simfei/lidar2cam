    ...
    def _get_XYZ_for_1frame(self):
        ...
        if self.range_field_a.shape[1] != 1024:
            ...
            data = np.concatenate([self.range_field_a.reshape(1,-1), self.signal_field_a.reshape(1,-1), 
                                   self.near_ir_field_a.reshape(1,-1), self.reflectivity_field_a.reshape(1,-1)], axis=0)
...
if __name__ == '__main__':
    #rosbag_file = 'CamerasAndLidars_2021-08-20-18-40-42_filtered.bag'
    rosbag_file = 'CamerasAndLidars_2021-08-27-12-59-11.bag'
    topic = '/os_node/lidar_packets' 
    metadata_path = 'os-992031000052.local.json'
    extractor = RosbagToPCLExtractor(rosbag_file, topic, metadata_path)
    extractor.preprocess_rosbag(show_frame_id=100)
